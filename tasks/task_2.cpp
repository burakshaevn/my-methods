...
public:
    ...
    bool AreSynonyms(const string& first_word, const string& second_word) const {
        for (const auto& [word, synonyms_set] : synonyms_) {
            if (first_word == word) {
                if (count(synonyms_set.begin(), synonyms_set.end(), second_word))
                    return true;
            }
            else if (second_word == word) {
                if (count(synonyms_set.begin(), synonyms_set.end(), first_word))
                    return true;
            }
        }
        /*
        или использовать алгоритм sort
        if (std::count(synonyms_.at(first_word).begin(), synonyms_.at(first_word).end(), second_word)) return true;
        */
        return false;
    }

private:
    map<string, set<string>> synonyms_; // <слово, <множество синонимов к нему>>
};
